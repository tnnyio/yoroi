...
## Using JSON RPC with tnnyio/yoroi
Using JSON RPC and tnnyio/yoroi together is quite simple.
...
Each JSON RPC _method_ is implemented as an `EndpointCodec`, a tnnyio/yoroi [Endpoint](https://godoc.org/github.com/tnnyio/yoroi/endpoint#Endpoint), sandwiched between a decoder and encoder. The decoder picks apart the JSON RPC request params, which can be passed to your endpoint. The encoder receives the output from the endpoint and encodes a JSON-RPC result.
