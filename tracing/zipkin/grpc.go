...
func GRPCClientTrace[I, O interface{}](tracer *zipkin.Tracer, options ...TracerOption) grpcTransport.ClientOption[I, O] {
	...
	clientBefore := grpcTransport.ClientBefore[I, O](
		func(ctx context.Context, md *metadata.MD) context.Context {
			...
			if config.name != "" {
				...
				name = ctx.Value(grpcTransport.ContextKeyRequestMethod).(string)
			}
	...
	clientAfter := grpcTransport.ClientAfter[I, O](
	...
	clientFinalizer := grpcTransport.ClientFinalizer[I, O](
	...
	return func(c *grpcTransport.Client[I, O]) {
		clientBefore(c)
		clientAfter(c)
		clientFinalizer(c)
	}
...
func GRPCServerTrace[I, O interface{}](tracer *zipkin.Tracer, options ...TracerOption) grpcTransport.ServerOption[I, O] {
	config := tracerOptions{
		tags:      make(map[string]string),
		name:      "",
		logger:    log.NewNopLogger(),
	...
	serverBefore := grpcTransport.ServerBefore[I, O](
		func(ctx context.Context, md metadata.MD) context.Context {
			var (
				spanContext model.SpanContext
				name        string
			...
			rpcMethod, ok := ctx.Value(grpcTransport.ContextKeyRequestMethod).(string)
			if !ok {
	...
	serverAfter := grpcTransport.ServerAfter[I, O](
		func(ctx context.Context, _ *metadata.MD, _ *metadata.MD) context.Context {
			if span := zipkin.SpanFromContext(ctx); span != nil {
				span.Finish()
			}
	...
	serverFinalizer := grpcTransport.ServerFinalizer[I, O](
		func(ctx context.Context, err error) {
			if span := zipkin.SpanFromContext(ctx); span != nil {
				if err != nil {
	...
	return func(s *grpcTransport.Server[I, O]) {
		serverBefore(s)
		serverAfter(s)
		serverFinalizer(s)
	}
