...
func HTTPClientTrace[I, O interface{}](tracer *zipkin.Tracer, options ...TracerOption) httpTransport.ClientOption[I, O] {
	...
	clientBefore := httpTransport.ClientBefore[I, O](
	...
	clientAfter := httpTransport.ClientAfter[I, O](
	...
	clientFinalizer := httpTransport.ClientFinalizer[I, O](
	...
	return func(c *httpTransport.Client[I, O]) {
		clientBefore(c)
		clientAfter(c)
		clientFinalizer(c)
	}
...
func HTTPServerTrace[I, O interface{}](tracer *zipkin.Tracer, options ...TracerOption) httpTransport.ServerOption[I, O] {
	config := tracerOptions{
		tags:      make(map[string]string),
		name:      "",
		logger:    log.NewNopLogger(),
	...
	serverBefore := httpTransport.ServerBefore[I, O](
		func(ctx context.Context, req *http.Request) context.Context {
			var (
				spanContext model.SpanContext
				name        string
	...
	serverAfter := httpTransport.ServerAfter[I, O](
	...
	serverFinalizer := httpTransport.ServerFinalizer[I, O](
		func(ctx context.Context, code int, r *http.Request) {
			if span := zipkin.SpanFromContext(ctx); span != nil {
				...
				if rs, ok := ctx.Value(httpTransport.ContextKeyResponseSize).(int64); ok {
	...
	return func(s *httpTransport.Server[I, O]) {
		serverBefore(s)
		serverAfter(s)
		serverFinalizer(s)
	}
