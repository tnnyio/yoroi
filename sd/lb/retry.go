...
func Retry(max int, timeout time.Duration, b Balancer) endpoint.Endpoint[any] {
...
// first.
func RetryWithCallback(timeout time.Duration, b Balancer, cb Callback) endpoint.Endpoint[any] {
	if cb == nil {
		cb = alwaysRetry
	}
	if b == nil {
